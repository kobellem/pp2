...
(define z21Handler%
  (class object%
    ...
    (define (get-positions)
      ...
      (let* ([answer (thread-send listenThread "get-answer")]
    ...
    (define (set-speed args)
      (let* ([id (car args)]
        ...
        (send socket msg)
        (println (thread-send listenThread "get-answer"))))
    (define (switch-state args)
      (let* ([id (car args)]
             [msg (z21:make-get-switch-info-msg (number->string (- id 1)) "00")])
        (send socket msg))
      (let ([answer (thread-send listenThread "get-answer")])
    ...
    (define (switch args)
      (let* ([id (car args)]
             ...
             [msg (z21:make-set-switch-msg (number->string (- id 1)) "00" true state-n)])
        (send socket msg)
        (println (thread-send listenThread "get-answer"))))
    ...
    (define (make-listenThread)
      (thread (lambda ()
        ...
        (let loop ([msg (thread-receive)])
          (cond
            [(string=? msg "get-answer")
              (let inner-loop ()                ;check if the answer has been received already
                (if answer answer (sleep 0.1))  ;if not, sleep for 0.1 and check again
                (inner-loop))]
          [else "Unknwonw message"])
        (set! answer #f)
        (loop (thread-receive))))))
))
